...
double X[M],Y[M],Z[M] ; //points in 3d space
double Xplot[M],Yplot[M] ; // project points
int N = 0 ;
...
int project(double observer_distance, double halfangle_degrees)
{
  ...
  double har = halfangle_degrees * M_PI/180;
  ...
  for(int i = 0; i < N ; i++){
    xprime = D * X[i] / (D + Z[i]);
    yprime = D * Y[i] / (D + Z[i]);
    xhat = ((Wsize*0.5)/H) * xprime + (Wsize*0.5);
    yhat = ((Wsize*0.5)/H) * yprime + (Wsize*0.5);
    Xplot[i] = xhat;
    Yplot[i] = yhat;
  }
}
...
int build_H() {
  ...
  for (k = 0; k < n ; k++) {
    save_line(x[k],-0.25,z[k],    x[k+1],-0.25,z[k+1]) ;  //bottom square
    save_line(x[k],0.25,z[k],    x[k+1],.25,z[k+1]) ;  //top square
    save_line(x[k],-0.25,z[k],    x[k],0.25,z[k]) ;  //connect lines
