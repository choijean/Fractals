...
int three_d_line(double xs, double ys, double zs,
                 double xe, double ye, double ze,
	         double observer_distance, double halfangle_degrees)
{
  double D,tanhalf,Whalf ;
  D = observer_distance ;
  tanhalf = tan( halfangle_degrees*M_PI/180 ) ;
  ...
  for (int i = 0 ; i<= n ; i++) {
    ...
    xp = (x/(D + z))  * (Whalf/tanhalf) + Whalf ;
    yp = (y/(D + z))  * (Whalf/tanhalf) + Whalf ;
...
int three_d_draw(double observer_distance, double halfangle_degrees)
{
  int i ;
  for (i = 0 ; i < N ; i=i+2) {
    three_d_line(X[i],Y[i],Z[i],  X[i+1],Y[i+1],Z[i+1],
		     observer_distance, halfangle_degrees) ;
  }
}
